...
import com.proyecto.service.IntegranteFamiliaService;
...
@Tag(name = "IntegranteFamilia")
@RequestMapping("${application.api.path}/integrantes-familia")
public class IntegranteFamiliaController {
    private final IntegranteFamiliaService servicio;

    @PostMapping
    @Operation(summary = "Guardar IntegranteFamilia")
    @PreAuthorize("hasAnyRole('ADMIN','OPERADOR')")
    public ResponseEntity<?> guardar(@Valid @RequestBody IntegranteFamiliaDto dto) {
        IntegranteFamiliaDto guardado = servicio.guardar(dto);
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(RespuestaDto.correcta("Se guardo correctamente", guardado));
    }

    @PutMapping("/{id}")
    @Operation(summary = "Editar IntegranteFamilia; requiere version actual")
    @PreAuthorize("hasAnyRole('ADMIN','OPERADOR')")
    public RespuestaDto<?> editar(
            @PathVariable Long id, @Valid @RequestBody IntegranteFamiliaDto dto) {
