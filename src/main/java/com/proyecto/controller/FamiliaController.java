...
import com.proyecto.service.FamiliaService;
...
@Tag(name = "Familia")
@RequestMapping("${application.api.path}/familias")
public class FamiliaController {
    private final FamiliaService servicio;

    @PostMapping
    @Operation(summary = "Guardar Familia")
    @PreAuthorize("hasAnyRole('ADMIN','OPERADOR')")
    public ResponseEntity<?> guardar(@Valid @RequestBody FamiliaDto dto) {
        FamiliaDto guardado = servicio.guardar(dto);
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(RespuestaDto.correcta("Se guardo correctamente", guardado));
    }

    @PutMapping("/{id}")
    @Operation(summary = "Editar Familia; requiere version actual")
    @PreAuthorize("hasAnyRole('ADMIN','OPERADOR')")
    public RespuestaDto<?> editar(@PathVariable Long id, @Valid @RequestBody FamiliaDto dto) {
