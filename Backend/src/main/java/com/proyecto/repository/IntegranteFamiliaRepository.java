...
public interface IntegranteFamiliaRepository extends JpaRepository<IntegranteFamilia, Long> {
    ...
    @Query("select i from IntegranteFamilia i where i.idBeneficiario = :idBeneficiario and i.activo = true order by i.id")
    List<IntegranteFamilia> findActivosByIdBeneficiario(
            @org.springframework.data.repository.query.Param("idBeneficiario") Long idBeneficiario);

    @Query("select i from IntegranteFamilia i where i.idFamilia in (select t.idFamilia from IntegranteFamilia t where t.idBeneficiario = :idBeneficiario and t.activo = true) and i.activo = true order by i.id")
    List<IntegranteFamilia> findNucleoFamiliarActivo(
            @org.springframework.data.repository.query.Param("idBeneficiario") Long idBeneficiario);
}
