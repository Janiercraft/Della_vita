...
import com.proyecto.service.ParticipacionService;
...
@RequestMapping("${application.api.path}/participaciones")
public class ParticipacionController {
    private final ParticipacionService servicio;

    @PostMapping
    @Operation(summary = "Guardar Participacion")
    @PreAuthorize("hasAnyRole('ADMIN','OPERADOR')")
    public ResponseEntity<?> guardar(@Valid @RequestBody ParticipacionDto dto) {
        ParticipacionDto guardado = servicio.guardar(dto);
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(RespuestaDto.correcta("Se guardo correctamente", guardado));
    }

    @PutMapping("/{id}")
    @Operation(summary = "Editar Participacion; requiere version actual")
    @PreAuthorize("hasAnyRole('ADMIN','OPERADOR')")
    public RespuestaDto<?> editar(@PathVariable Long id, @Valid @RequestBody ParticipacionDto dto) {
