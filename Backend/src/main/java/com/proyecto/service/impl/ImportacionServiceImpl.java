...
public class ImportacionServiceImpl implements ImportacionService {
    ...
    @Override
    public ImportacionDto cargarAutomatico(
            MultipartFile archivo, boolean confirmar, int hoja, String separador) {
        ConfiguracionImportacionDto configuracion = ConfiguracionImportacionDto.builder()
                .mapeo(new LinkedHashMap<>())
                .hoja(Math.max(0, hoja))
                .separador(separador == null || separador.isBlank() ? "AUTO" : separador)
                .build();
        return cargar(archivo, configuracion, confirmar);
    }

    @Override
    public AnalisisColumnasImportacionDto analizar(
            MultipartFile archivo, int hoja, String separador) {
        try {
            String nombre = archivo.getOriginalFilename();
            if (nombre == null || nombre.isBlank()) {
                throw ExcepcionNegocio.invalido("Nombre de archivo invalido");
            }
            if (archivo.isEmpty() || archivo.getSize() > 10L * 1024 * 1024) {
            ...
            ConfiguracionImportacionDto configuracion = ConfiguracionImportacionDto.builder()
                    ...
                    .separador(separador == null || separador.isBlank() ? "AUTO" : separador)
                    .build();
            LectorArchivoService lector = lectores.stream()
                    .filter(actual -> actual.soporta(nombre))
                    .findFirst()
                    .orElseThrow(() -> ExcepcionNegocio.invalido(
                            "Formato no soportado: use CSV, XLSX o XLS"));
            List<Map<String, String>> filas = lector.leer(archivo.getBytes(), configuracion);
            if (filas.isEmpty()) {
                throw ExcepcionNegocio.invalido("El archivo no contiene filas de datos");
            }
            List<String> columnas = filas.getFirst().keySet().stream()
