...
public class UsuarioServiceImpl implements UsuarioService {
    ...
    public UsuarioDto guardar(UsuarioDto dto) {
        ...
        validarCorreo(dto.getCorreo(), null);
    ...
    public UsuarioDto editar(Long id, UsuarioDto dto) {
        ...
        validarCorreo(dto.getCorreo(), id);
    ...
    private void validarNombre(String nombre, Long id) {
        Optional<Usuario> existente = repository.findByNombreUsuarioIgnoreCase(nombre);
    ...
    private void copiar(UsuarioDto dto, Usuario usuario) {
        ...
        usuario.setCorreo(normalizarCorreo(dto.getCorreo()));
    ...
    private void validarCorreo(String correo, Long id) {
        String normalizado = normalizarCorreo(correo);
        if (normalizado == null) {
            return;
        }
        Optional<Usuario> existente = repository.findByCorreoIgnoreCase(normalizado);
        if (existente.isPresent() && !existente.get().getId().equals(id)) {
            throw ExcepcionNegocio.conflicto("El correo ya esta asociado a otro usuario");
        }
    }
    ...
    private Long validarBeneficiarioPorRol(String rol, Long idBeneficiario) {
        if ("CONSULTA".equals(rol)) {
            if (idBeneficiario == null) {
                throw ExcepcionNegocio.invalido("Para el rol Beneficiario debe seleccionar una persona registrada en la plataforma");
            }
