...
public class LectorCsvServiceImpl implements LectorArchivoService {
    ...
    public List<Map<String, String>> leer(
        ...
        try {
            ...
            CSVFormat formato =
                    CSVFormat.DEFAULT
                            .builder()
                            .setDelimiter(detectarSeparador(texto, configuracion.getSeparador()))
                            .setHeader()
    ...
    private char detectarSeparador(String texto, String configurado) {
        if (configurado != null && !configurado.isBlank() && !configurado.equalsIgnoreCase("AUTO")) {
        ...
        for (char candidato : candidatos) {
            ...
            for (int i = 0; i < primera.length(); i++) {
                char actual = primera.charAt(i);
                if (actual == '"') {
                    comillas = !comillas;
                } else if (!comillas && actual == candidato) {
                    conteo++;
                }
            }
            if (conteo > max) {
    ...
    public static void validarEncabezados(
        ...
        Map<String, String> mapeo = configuracion.getMapeo() == null ? Map.of() : configuracion.getMapeo();
        for (String columna : mapeo.values()) {
