...
import { EventManagementView } from './modules/events/EventManagementView';
...
const ROUTE_CONFIG = {
  ...
  '#/usuarios': { tab: 'users', minRole: 'coordinator' },
  '#/eventos': { tab: 'events', minRole: 'coordinator' },
...
function MainLayout() {
  ...
  const canApproveConflicts = role === 'admin' || role === 'coordinador' || role === 'profesional';
  ...
  useEffect(() => {
    const evaluateRouteGuard = () => {
      ...
      // C) Gestión de usuarios y roles: exclusiva de Coordinación
      if (matchedRoute.minRole === 'coordinator' && role !== 'admin' && role !== 'coordinador') {
        window.location.hash = '#/beneficiarios';
        setActiveTab('beneficiaries');
        setSecurityAlert({
          message: 'Acceso restringido: solo Coordinación puede administrar usuarios y asignar roles.',
          timestamp: Date.now()
        });
        setTimeout(() => setSecurityAlert(null), 6000);
        return;
      ...
      // D) Ruta autorizada: sincroniza la pestaña activa
  ...
  const getTabTitle = () => {
    ...
    switch (activeTab) {
      ...
      case 'users':
        return 'Usuarios, Roles y Accesos';
      case 'events':
          ...
          <ErrorBoundary key={`${role}-${activeTab}`}>
            {role === 'usuario' ? (
              ...
              <>
                ...
                {activeTab === 'users' && (role === 'admin' || role === 'coordinador') && <UserManagementView />}
                {activeTab === 'events' && (role === 'admin' || role === 'coordinador') && <EventManagementView />}
              </>
            )}
          </ErrorBoundary>
        </main>
