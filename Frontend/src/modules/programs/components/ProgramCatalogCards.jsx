...
export function ProgramCatalogCards({
  ...
  const getTarget = (id) => {
    const matching = programs.filter((p) => p.lineId === id && Number.isFinite(Number(p.targetEstimated)));
    if (matching.length) {
      const total = matching.reduce((sum, p) => sum + Number(p.targetEstimated || 0), 0);
      return `~${new Intl.NumberFormat('es-CO').format(total)} personas`;
    }
    switch (id) {
  ...
  const getOrganizations = (id, fallback) => {
    const orgs = [...new Set(programs.filter((p) => p.lineId === id).map((p) => p.leadOrg).filter(Boolean))];
    return orgs.length ? orgs.join(' / ') : fallback;
  };
        ...
        return (
          ...
          >
            ...
            <div style={{ fontSize: '0.78rem', color: 'var(--color-teal-text)', fontWeight: 600, marginBottom: '0.35rem' }}>
              Org: {getOrganizations(line.id, line.org)}
            </div>
