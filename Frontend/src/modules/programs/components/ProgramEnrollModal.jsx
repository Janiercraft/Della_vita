...
import { ENROLLMENT_STATUSES, checkDuplicateEnrollment } from '../../../core/domain/programRules';
...
export function ProgramEnrollModal({
  ...
  const [formData, setFormData] = useState({
    beneficiaryId: '',
    programId: programsCatalog[0]?.id || '',
  ...
  useEffect(() => {
    if (isOpen) {
      beneficiaryRepository.getAll().then(setBeneficiariesList).catch(() => setBeneficiariesList([]));

      const initialBenId = preselectedBeneficiaryId || '';
      const initialProgId = programsCatalog[0]?.id || '';
  ...
  }, [isOpen, preselectedBeneficiaryId, existingEnrollments, programsCatalog]);
  ...
  const programOptions = programsCatalog.map(p => ({
    value: p.id,
  ...
  return (
      ...
      footer={
        <>
          ...
          <Button
            ...
            disabled={!!duplicateWarning || !formData.beneficiaryId || !formData.programId}
          >
    ...
    >
      <form onSubmit={handleSubmit}>
        ...
        {programsCatalog.length === 0 && (
          <div className="alert alert-warning" style={{ marginBottom: '1rem' }}>
            No hay programas activos disponibles en la API. Verifique que Flyway haya aplicado la migración V10.
          </div>
        )}
